    ...
    for i, result_temp in enumerate(results):
        ...
        for result in result_temp[0]:
            ...
            if conf >= bbox_thresh:
                ...
                bbox = np.array(bbox, dtype=np.int32)
                if i not in data_args['pts_center_index']:
                    cv2.rectangle(image, (bbox[0], bbox[1]), (bbox[2], bbox[3]), c, 1)
                    cv2.rectangle(image, (bbox[0], bbox[1] - cat_size[1] - 2), (bbox[0] + cat_size[0], bbox[1] - 2), c, -1)
                    cv2.putText(image, txt, (bbox[0], bbox[1] - 2), font, 0.5, (0, 0, 0), thickness=1, lineType=cv2.LINE_AA)
                else:
                    cv2.putText(image, txt, (bbox[0], bbox[1] - 50), font, 0.5, (0, 0, 0), thickness=1, lineType=cv2.LINE_AA)
                ...
                for j in range(0, hps_merge.size, 3):
                    if hps_merge[j + 2] >= pt_thresh:
                        ...
                        cv2.putText(image, str(j // 3), (int(hps_merge[j]), int(hps_merge[j + 1])), font, 0.3, c,
                                    thickness=1, lineType=cv2.LINE_AA)
                        cv2.putText(image, str(hps_merge[j + 2])[:3], (int(hps_merge[j]), int(hps_merge[j + 1])-10),
                                    font, 0.3, c, thickness=1, lineType=cv2.LINE_AA)
