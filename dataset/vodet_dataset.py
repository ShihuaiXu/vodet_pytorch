...
class Vodet_Dataset(Dataset):
    ...
    def __init__(self, split, data_args):
        ...
        self.use_multi_center = data_args['use_multi_center']
                ...
                if label in self.pts_center_index:
                    ...
                    if count >= 2:
                        ...
                        if self.use_multi_center:
                            for j in range(4):
                                if j == 0:
                                    ct_int = (np.array([int(ct[0]), int(ct[1])], dtype=np.int32))
                                elif j == 1:
                                    ct_int = (np.array([np.ceil(ct[0]), int(ct[1])], dtype=np.int32))
                                elif j == 2:
                                    ct_int = (np.array([int(ct[0]), np.ceil(ct[1])], dtype=np.int32))
                                elif j == 3:
