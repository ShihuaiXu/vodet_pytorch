...
from model_tools.vodet_show_results_tool import show_results
...
parser.add_argument('--out_img_path', default='output_imgs', help='output imgs path')
parser.add_argument('--gpus', default='0', help='-1 for CPU, use comma for multiple gpus')
parser.add_argument('--demo', default='', help='path to image/image folders/json file')
parser.add_argument('--image_path', default='', help='image path of demo when input json file')
...
if not os.path.exists(args.demo):
    assert 0, 'demo path is not exists!!!'

if args.image_path != '' and not os.path.exists(args.image_path):
    assert 0, 'img path is not exists!!!'

if not os.path.isdir(args.out_img_path):
    os.makedirs(args.out_img_path)
...
def demo():
    ...
    model.eval()
    ##############################
    image_names = get_images(args.demo, args.image_path)

    for index, image_name in enumerate(image_names):
        print(str(index + 1) + '/' + str(len(image_names)))
        out_full_path = os.path.join(args.out_img_path, image_name.rsplit('/', 1)[-1])
        # data preprocess
        ...
        show_results(data_args, out_full_path, image_draw, ctdet_results, multi_results, args.bbox_thresh,
                     args.pts_thresh)
